...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initRoles() {
        ...
        // 插入基础角色
        String sql = "INSERT INTO tb_role (role_name, role_key, sort_order, status, remark, created_time, updated_time) VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    private void initPermissions() {
        ...
        String sql = "INSERT INTO tb_permission (permission_name, permission_key, permission_type, sort_order, status, remark, created_time, updated_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        LocalDateTime now = LocalDateTime.now();
    ...
    private void initRolePermissions() {
        ...
        if (adminRoleId != null) {
            // 为ADMIN角色分配所有权限
            String sql = "INSERT INTO tb_role_permission (role_id, permission_id, created_time) SELECT ?, id, ? FROM tb_permission";
    ...
    private void initAdminUser() {
        ...
        if (userCount != null && userCount > 0) {
            ...
            jdbcTemplate.update("UPDATE tb_user SET password = ?, updated_time = ? WHERE username = 'admin'",
                               encodedPassword, LocalDateTime.now());
            return;
        }
        
        // 创建admin用户
        String sql = "INSERT INTO tb_user (username, password, real_name, email, phone, status, created_time, updated_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        String encodedPassword = passwordEncoder.encode("admin123");
        LocalDateTime now = LocalDateTime.now();
    ...
    private void initUserRoles() {
        ...
        if (adminUserId != null && adminRoleId != null) {
            String sql = "INSERT INTO tb_user_role (user_id, role_id, created_time) VALUES (?, ?, ?)";
