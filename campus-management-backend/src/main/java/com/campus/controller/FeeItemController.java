...
@Tag(name = "缴费项目管理", description = "缴费项目的增删改查操作")
public class FeeItemController {
    ...
    public ResponseEntity<ApiResponse<FeeItemRepository.FeeItemDetail>> getFeeItemDetail(
            @Parameter(description = "缴费项目ID") @PathVariable Long id) {
        try {
            Optional<FeeItemRepository.FeeItemDetail> detail = feeItemService.findDetailById(id);
            if (detail.isPresent()) {
