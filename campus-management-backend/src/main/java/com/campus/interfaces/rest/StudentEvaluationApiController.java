...
public class StudentEvaluationApiController {
    ...
    public ResponseEntity<ApiResponse<StudentEvaluation>> createEvaluation(
        ...
        try {
            logger.info("创建学生评价: studentId={}, evaluatorId={}, type={}", 
    ...
    public ResponseEntity<ApiResponse<StudentEvaluation>> getEvaluationById(
        ...
        try {
            ...
            logger.error("获取学生评价详情失败: evaluationId={}", evaluationId, e);
            return ResponseEntity.badRequest()
    ...
    public ResponseEntity<ApiResponse<StudentEvaluation>> updateEvaluation(
            ...
            @Valid @RequestBody StudentEvaluation evaluation) {
        try {
            logger.info("更新学生评价: evaluationId={}", evaluationId);
    ...
    @PostMapping("/batch")
    @Operation(summary = "批量创建评价", description = "批量创建学生评价")
    public ResponseEntity<ApiResponse<List<StudentEvaluation>>> batchCreateEvaluations(
            @RequestBody List<StudentEvaluation> evaluations) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success("批量创建评价成功", result));
            
        } catch (Exception e) {
            logger.error("批量创建评价失败", e);
            return ResponseEntity.badRequest()
                .body(ApiResponse.error("批量创建评价失败: " + e.getMessage()));
        }
    }
    
    @DeleteMapping("/batch")
    @Operation(summary = "批量删除评价", description = "批量删除学生评价")
    public ResponseEntity<ApiResponse<Void>> batchDeleteEvaluations(
            @RequestBody List<Long> evaluationIds) {
        try {
            logger.info("批量删除学生评价: {} 个", evaluationIds.size());
            
            studentEvaluationService.batchDeleteEvaluations(evaluationIds);
            return ResponseEntity.ok(ApiResponse.success("批量删除评价成功"));
            
        } catch (Exception e) {
            logger.error("批量删除评价失败", e);
            return ResponseEntity.badRequest()
                .body(ApiResponse.error("批量删除评价失败: " + e.getMessage()));
        }
    }
